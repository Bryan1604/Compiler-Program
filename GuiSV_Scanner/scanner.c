...
Token* readIdentKeyword(void){
  ...
  {
    if(count <=MAX_IDENT_LEN) toupper((char)currentChar);
  }
...
Token* getToken(void) 
{
  ...
  {
  case 0:
  	if (currentChar == EOF) state =1;  // TK_EOF
  	else
  	switch (charCodes[currentChar]) 
  	{
  		case CHAR_SPACE: 
  ...
  case 2:
    //TODO (Skip blanks)  // gan token   -> get(token)
    skipBlank();
    return getToken();  // need to check again
  case 3:
 	// TODO Recognize Identifiers and keywords
    return readIdentKeyword();
  case 4:    // sau khi doc
  		token->tokenType = checkKeyword(str);   // checkyeword : trar ra token type
  ...
  case 6:
  		//TODO Keywords
      return makeToken(KW_TYPE, ln,cn);
  case 7:   // odc het chu so thi thoi 
	//TODO  Numbers
    return readNumber();
  case 8: 
  // return kt_number
